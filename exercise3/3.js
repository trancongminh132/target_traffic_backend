/**
 * Modify the regular expressions to match the sentence/comment.
 */
if ("This sentense does not start with a digit like 1,2,3,4".match(new RegExp("^([1-4])"))) {
    throw new Error("1");
}
if ("This sentense does end with a digit like 1,2,3,4".match(new RegExp("^[a-z|A-Z|0-9]+[1-4]$"))) {
    throw new Error("2");
}
//Match every m that is NOT followed by a letter from j to z
var rex = new RegExp("^[m]+[j-z]$");
if ("ma".match(rex)[1] != "m" || "mz".match(rex) != null) {
    throw new Error("3");
}
//# sourceMappingURL=data:application/json;base64,